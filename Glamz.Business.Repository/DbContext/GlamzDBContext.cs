...
{
    public class GlamzDBContext : IGlamzDBContext
    {
        protected IMongoDatabase _database;

        public GlamzDBContext()
        {

        }
        public GlamzDBContext(string connectionString)
        {
        ...
        public GlamzDBContext(IMongoDatabase mongodatabase)
        {
        ...
        public async Task CreateIndex<T>(IGlamzRepository<T> repository, OrderBuilder<T> orderBuilder, string indexName, bool unique = false) where T : BaseEntity
        {
            ...
            try
            {
                await ((GlamzRepository<T>)repository).Collection.Indexes.CreateOneAsync(new CreateIndexModel<T>(Builders<T>.IndexKeys.Combine(keys),
